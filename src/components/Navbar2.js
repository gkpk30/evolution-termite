...
import FlyoutSimpleMobile from "../components/FlyoutSimpleMobile";
          ...
          {navigation.map((item) =>
            item.subMenu ? (
              ...
              <Link
                to={item.href}
                key={item.name}
                className="first:rounded-bl-xl text-sm font-semibold items-center leading-6 bg-gray-800/60 hover:bg-gray-700 text-gray-50 py-4 px-6"
              >
                {item.name}
              </Link>
            )
        ...
        <Dialog.Panel className="fixed inset-y-0 right-0 z-50 w-full overflow-y-auto bg-gray-300 px-6 py-6 sm:max-w-sm sm:ring-1 sm:ring-gray-900/10">
          <div className="flex items-center justify-between">
            <Link to="/" className="-m-1.5 p-1.5">
              <span className="sr-only">Evolution Termite</span>
              <img
                  ...
                  item.subMenu ? (
                    ...
                    // <button  onClick={() => setMobileMenuOpen(false)}>
                      <Link
                        key={item.name}
                        to={item.href}
                        className="-mx-3  inline-flex rounded-xl py-2 px-3 text-base font-semibold leading-7 text-gray-900 hover:bg-gray-400"
                      >
                        {item.name}
                      </Link>
                    // </button>
